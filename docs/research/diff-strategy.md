...
表格渲染仍使用同一份“基线 Markdown 原文 ↔ 当前 Markdown 原文”结果，不比较渲染 DOM。表格工具栏的行增删另外记录轻量 transaction provenance：新增行保持绿色身份；删除行保存其基线行范围和当前 gap 锚点。它只用于拆开快照本身无法判定的“不等长替换块”（例如删两行并修改相邻一行），并随 CodeMirror changes 映射、undo/redo 反转、基线刷新清空。渲染层仅把最终范围投影到可见表格行，仍不参与重新配对。
