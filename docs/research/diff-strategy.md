...
表格渲染仍使用同一份“基线 Markdown 原文 ↔ 当前 Markdown 原文”结果，不比较渲染 DOM。表格工具栏的行增删另外记录轻量 transaction provenance：新增行保持绿色身份；删除行保存其基线行范围和当前 gap 锚点。它只用于拆开快照本身无法判定的“不等长替换块”（例如删两行并修改相邻一行），并随 CodeMirror changes 映射、undo/redo 反转、基线刷新清空。若重复空行让快照 Diff 选中了等价位置，或把多次删除聚合成一个范围，事务来源会在同一表格内逐基线行替换内容相同的快照 gap，支持一对多、多对一和多对多，避免重复显示。渲染层仅把最终范围投影到可见表格行，仍不参与重新配对。
...
当前表格工具栏的行增删已采用上述独立 provenance 层，因为事务发生在编辑器内部且用户明确要求保留红/绿操作语义；普通键盘输入和外部文档同步仍只遵循快照 diff，不猜测历史操作。
