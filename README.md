...
- 新增工具栏行号跳转和编辑模式记忆；输入行号后按 `Enter` 即可跳转。
...
- 统一代码块、Mermaid 和块级公式在 Live、Preview 与导出结果中的圆角样式。
- 优化 Live、Source 与 Preview 相互切换时的视口保持；Preview 与 HTML/PDF 导出共用深浅色阅读样式，仅保留必要的容器差异。
- 优化查找控件、工具栏、模式切换器和滚动条的布局与样式。
- 优化窄窗口下的工具栏显示：优先保留右侧操作与模式切换，左侧只显示完整控件，并以不可点击的白色省略号提示其余内容。
- 优化行号跳转框的文字对齐；Live / Source / Preview 与 Light / Dark 切换器统一采用全圆角、按文字动态分配宽度的样式。
...
- 修复表格交互状态残留时，粗体、删除线、链接和图片等 Markdown 标记点击后偶发无法展开的问题。
- 修复固定基线按钮偶发引起编辑内容上下抖动的问题。
- 修复 Mermaid 分栏编辑时较短一侧未随页面保持可见的问题。
- 修复 Preview 标题下方多余分割线，以及 Preview 与导出结果样式不一致的问题。
- 修复超高 Mermaid 导出 PDF 时产生空白背景页和跨页截断的问题。
